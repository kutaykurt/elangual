...
const HiraganaAlphabet = () => {
  ...
  const renderAlphabetForPage = () => {
    ...
    return japaneseData.hiraganaAlphabet
      ...
      .map((item, index) => {
        const id = generateVocabularyId(item);
        const isSelected = hiraganaVocabularyList.some(
          (vocab) => vocab.id === id
        );
              ...
              <td>
                <button
                  onClick={(e) => {
                    ...
                    if (isSelected) {
                      dispatch(
                        removeVocabulary({ id, scriptType: "hiragana" })
                      );
                    } else {
                      dispatch(
                        addVocabulary({
                          newVocabulary: { ...item, id },
                          scriptType: "hiragana",
                        })
                      );
                    }
        ...
        >
          ...
          <Modal.Footer>
            ...
            <Button
              variant="primary"
              onClick={() => {
                const id = generateVocabularyId(selectedHiraganaItem);
                const isSelected = hiraganaVocabularyList.some(
                  (vocab) => vocab.id === id
                );

                if (isSelected) {
                  dispatch(removeVocabulary({ id, scriptType: "hiragana" }));
                } else {
                  dispatch(
                    addVocabulary({
                      newVocabulary: { ...selectedHiraganaItem, id },
                      scriptType: "hiragana",
                    })
                  );
                }
            ...
            >
              {hiraganaVocabularyList.some(
                (vocab) =>
                  vocab.id === generateVocabularyId(selectedHiraganaItem)
              )
                ? "Remove"
                : "Add to Vocabulary"}
            </Button>
